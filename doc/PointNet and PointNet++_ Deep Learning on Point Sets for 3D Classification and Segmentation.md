...
首先，回顾MRG方法：其为了避免MSG方法对于每层的点云都采用多个尺寸的局部区域进行特征聚合所造成的庞大计算开销同时保证模型的多尺度模式提取能力，而采用了通过连接不同层特征的方式以达到点云多尺度特征的聚合。
![f_mrg](/doc/img/f3.png)
为了复现MRG策略，我们参考了论文中的特征提取步骤的网络结构，如下图：
![f_structure](/doc/img/论文网络结构.png)
通过论文中的网络结构，我们实现了分类网络的MRG策略，如下图（SA为Set Abstraction；(N,D)表示输入点数为N，特征维度为D；C表示连接操作）：
![f_net](/doc/img/网络示意图.png)
其分为了四个分支，分支一经过两层的特征提取得到（64，256）的特征，与经过一层特征提取的分支二输出的（512，256）进行拼接，拼接后的特征会再经过分支四的一层特征提取后与分支三输出的特征进行拼接，最终得到尺寸为（1，1536）的多尺度特征。此多尺度特征即可作为后续网络的输入进行分类任务。
